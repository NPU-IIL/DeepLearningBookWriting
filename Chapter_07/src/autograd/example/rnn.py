...
# 辅助函数
def sigmoid(x):
    return 0.5*(np.tanh(x) + 1.0)   # 输出范围：0-1
...
def build_dataset(filename, sequence_length, alphabet_size, max_lines=-1):
    """加载文本文件，并将每行转换为一个编码序列"""
    with open(filename) as f:
        content = f.readlines()
    content = content[:max_lines]
    content = [line for line in content if len(line) > 2]   # 删除空行
